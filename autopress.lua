...
local function createDisplay()
    ...
    title.Text = "🤖 Auto Presser (Space Only - Cyclic)"
...
local function startContinuousPress(keyDisplay, counterText, timerText, statusText, dot, cycleText)
    ...
    while pressing and tick() < endTime do
        ...
        wait(0.02) -- 20ms delay while holding
...
local function startPressing(statusText, dot, keyDisplay, timerText, counterText, cycleText)
    ...
    print("🔴 Auto-press STARTED - Cycling: 5 seconds press, 3 seconds wait, repeat")
    
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "Auto-Presser",
        Text = "STARTED! 5s press → 3s wait → repeat | Press P to stop",
        Duration = 3
    })
end
...
local function initialize()
    ...
    print("📌 Pressing: SPACE only")
    print("⚡ Delay: 20ms between each press")
